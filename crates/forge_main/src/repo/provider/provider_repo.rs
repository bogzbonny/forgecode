...
use bytes::Bytes;
use crate::app::domain::{ProviderId, ProviderResponse};
use crate::app::{EnvironmentInfra, FileReaderInfra, FileWriterInfra, HttpInfra};
use crate::domain::{
...
enum Models {
    ...
    Hardcoded(Vec<crate::app::domain::Model>),
}
...
struct ProviderConfig {
    ...
    #[merge(strategy = merge::vec::append)]
    auth_methods: Vec<crate::domain::AuthMethod>,
    #[serde(default)]
    #[merge(strategy = overwrite)]
...
impl From<crate::config::ProviderUrlParam> for UrlParamVarConfig {
    fn from(param: crate::config::ProviderUrlParam) -> Self {
...
impl From<crate::config::ProviderEntry> for ProviderConfig {
    fn from(entry: crate::config::ProviderEntry) -> Self {
        let provider_type = match entry.provider_type {
            Some(crate::config::ProviderTypeEntry::ContextEngine) => {
                crate::domain::ProviderType::ContextEngine
            }
            Some(crate::config::ProviderTypeEntry::Llm) | None => crate::domain::ProviderType::Llm,
        };

        let auth_methods = if entry.auth_methods.is_empty() {
            vec![crate::domain::AuthMethod::ApiKey]
        } else {
            entry
                .auth_methods
                .into_iter()
                .map(|m| match m {
                    crate::config::ProviderAuthMethod::ApiKey => crate::domain::AuthMethod::ApiKey,
                })
                .collect()
        };

        let response_type = entry.response_type.map(|r| match r {
            crate::config::ProviderResponseType::OpenAI => ProviderResponse::OpenAI,
            crate::config::ProviderResponseType::OpenAIResponses => {
...
impl From<&ProviderConfig> for crate::domain::ProviderTemplate {
    fn from(config: &ProviderConfig) -> Self {
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => crate::domain::ModelSource::Url(
                crate::domain::Template::<crate::domain::URLParameters>::new(model_url_template),
            ),
            Models::Hardcoded(model_list) => {
                crate::domain::ModelSource::Hardcoded(model_list.clone())
            }
        });
        ...
        Provider {
            ...
            url: crate::domain::Template::<crate::domain::URLParameters>::new(&config.url),
...
impl<F: EnvironmentInfra<Config = crate::config::ForgeConfig> + HttpInfra>
...
> ForgeProviderRepository<F>
{
    ...
    async fn create_provider(
        &self,
        config: &ProviderConfig,
    ) -> anyhow::Result<crate::domain::ProviderTemplate> {
...
mod env_tests {
    ...
    use crate::domain::{AnyProvider, ChatRepository, ProviderTemplate};
    use pretty_assertions::assert_eq;
    use url::Url;
    ...
    impl EnvironmentInfra for MockInfra {
        ...
        async fn update_environment(
            &self,
            _ops: Vec<crate::domain::ConfigOperation>,
        ) -> anyhow::Result<()> {
            Ok(())
        }

        fn get_config(&self) -> anyhow::Result<crate::config::ForgeConfig> {
            Ok(crate::config::ForgeConfig::default())
        }
    ...
    impl FileReaderInfra for MockInfra {
        ...
        async fn range_read_utf8(
            ...
            _start_line: u64,
            _end_line: u64,
        ) -> anyhow::Result<(String, crate::domain::FileInfo)> {
            Err(anyhow::anyhow!("File not found"))
        }
    }
    ...
    impl ProviderRepository for MockInfra {
        ...
        async fn migrate_env_credentials(
            &self,
        ) -> anyhow::Result<Option<crate::domain::MigrationResult>> {
            Ok(None)
        }
    }
