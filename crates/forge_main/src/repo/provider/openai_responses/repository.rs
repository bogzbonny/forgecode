...
use crate::domain::{BoxStream, ChatRepository, Provider};
...
use super::super::FromDomain;
use super::super::retry::into_retry;
use super::super::utils::{create_headers, format_http_context};
...
impl<H: HttpInfra> OpenAIResponsesProvider<H> {
    ...
    pub fn new(provider: Provider<Url>, http: Arc<H>) -> Self {
        ...
        if provider.id == ProviderId::OPENAI_RESPONSES_COMPATIBLE
        {
                    ...
                    Ok(Event::Message(msg)) => {
                        ...
                        match result {
                            ...
                            Ok(super::response::ResponsesStreamEvent::Ping { cost }) => {
                                let usage =
                                    crate::domain::Usage { cost: Some(cost), ..Default::default() };
                                Some(Ok(super::response::StreamItem::Message(Box::new(
                                    ChatCompletionMessage::assistant(crate::domain::Content::part(
                                        "",
                                    ))
                                    .usage(usage),
                                ))))
    ...
    E: std::fmt::Debug + std::fmt::Display + Send + Sync + 'static,
{
    ...
    if is_retryable {
        crate::domain::Error::Retryable(error).into()
    } else {
        error
    }
}
    ...
    for OpenAIResponsesResponseRepository<F>
{
    ...
    async fn models(&self, provider: Provider<Url>) -> anyhow::Result<Vec<Model>> {
        match provider.models().cloned() {
            Some(crate::domain::ModelSource::Hardcoded(models)) => Ok(models),
            Some(crate::domain::ModelSource::Url(url)) => {
                ...
                let data: crate::app::dto::openai::ListModelResponse =
...
mod tests {
    ...
    use super::*;
    use crate::repo::provider::mock_server::MockServer;

    fn is_retryable(error: &anyhow::Error) -> bool {
        error
            .downcast_ref::<crate::domain::Error>()
            .is_some_and(|error| matches!(error, crate::domain::Error::Retryable(_)))
    }
    ...
    impl crate::app::EnvironmentInfra for MockHttpClient {
        ...
        fn get_environment(&self) -> crate::domain::Environment {
            use fake::{Fake, Faker};
            Faker.fake()
        }
    ...
    fn test_openai_responses_provider_new_with_oauth_with_api_key() {
        let provider = Provider {
            ...
            credential: Some(crate::domain::AuthCredential {
                id: ProviderId::OPENAI_COMPATIBLE,
                auth_details: crate::domain::AuthDetails::OAuthWithApiKey {
                    tokens: crate::domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
                    ),
                    api_key: crate::domain::ApiKey::from("oauth-key".to_string()),
                    config: crate::domain::OAuthConfig {
                        auth_url: Url::parse("https://example.com/auth").unwrap(),
                        token_url: Url::parse("https://example.com/token").unwrap(),
                        client_id: crate::domain::ClientId::from("client-id".to_string()),
                        scopes: vec![],
                        redirect_uri: None,
                        use_pkce: false,
    ...
    fn test_openai_responses_provider_new_with_oauth() {
        let provider = Provider {
            ...
            credential: Some(crate::domain::AuthCredential {
                id: ProviderId::OPENAI_COMPATIBLE,
                auth_details: crate::domain::AuthDetails::OAuth {
                    tokens: crate::domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
    ...
    fn test_get_headers_with_oauth_device_custom_headers() {
        let provider = Provider {
            ...
            credential: make_credential(ProviderId::OPENAI_COMPATIBLE, "test-key"),
            custom_headers: None,
            auth_methods: vec![crate::domain::AuthMethod::OAuthDevice(
                crate::domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: crate::domain::ClientId::from("client-id".to_string()),
                    scopes: vec![],
    ...
    fn test_get_headers_with_oauth_code_custom_headers() {
        let provider = Provider {
            ...
            credential: make_credential(ProviderId::OPENAI_COMPATIBLE, "test-key"),
            custom_headers: None,
            auth_methods: vec![crate::domain::AuthMethod::OAuthCode(
                crate::domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: crate::domain::ClientId::from("client-id".to_string()),
        ...
        let provider = Provider {
            ...
            auth_methods: vec![crate::domain::AuthMethod::OAuthDevice(
                crate::domain::OAuthConfig {
                    ...
                    client_id: crate::domain::ClientId::from("test_client".to_string()),
                    scopes: vec![],
                    redirect_uri: None,
                    use_pkce: false,
                    token_refresh_url: None,
    ...
    #[test]
    fn test_get_headers_non_openai_does_not_include_chatgpt_account_id() {
        let mut url_params = HashMap::new();
        url_params.insert(
            crate::domain::URLParam::from("chatgpt_account_id".to_string()),
            crate::domain::URLParamValue::from("acct_should_not_appear".to_string()),
        );
