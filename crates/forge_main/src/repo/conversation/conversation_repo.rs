...
use diesel::prelude::*;
use crate::domain::{Conversation, ConversationId, ConversationRepository, WorkspaceHash};

use super::conversation_record::ConversationRecord;
use crate::repo::database::schema::conversations;
use crate::repo::database::{DatabasePool, PooledSqliteConnection};
...
mod tests {
    ...
    use super::*;
    use super::super::conversation_record::{ContextRecord, MetricsRecord};
    use crate::repo::database::DatabasePool;
    ...
    fn test_context_record_conversion_preserves_all_fields() {
        ...
        let reasoning = crate::domain::ReasoningConfig {
        ...
        let messages = vec![
            ...
            .into(),
            crate::domain::MessageEntry {
                message: ContextMessage::Text(crate::domain::TextMessage {
                    role: Role::Assistant,
                    ...
                    tool_calls: Some(vec![ToolCallFull {
                        ...
                        arguments: crate::domain::ToolCallArguments::from(
                            serde_json::json!({"param": "value"}),
                        ),
                        thought_signature: None,
                    }]),
                    model: Some(crate::domain::ModelId::from("gpt-4")),
                    thought_signature: None,
                ...
                usage: Some(Usage {
                    prompt_tokens: crate::domain::TokenCount::Actual(100),
                    completion_tokens: crate::domain::TokenCount::Actual(50),
                    total_tokens: crate::domain::TokenCount::Actual(150),
                    cached_tokens: crate::domain::TokenCount::Actual(0),
                    cost: Some(0.001),
                }),
            },
        ];
        ...
        let fixture = Context::default()
            ...
            .temperature(crate::domain::Temperature::new(0.7).unwrap())
            .top_p(crate::domain::TopP::new(0.9).unwrap())
            .top_k(crate::domain::TopK::new(50).unwrap())
    ...
    fn test_legacy_tool_value_pair_deserialization() {
        use super::super::conversation_record::ToolOutputRecord;
        ...
        let actual: crate::domain::ToolOutput = record.try_into().unwrap();
        ...
        assert_eq!(
            actual.values[0],
            crate::domain::ToolValue::Text("XML content for LLM".to_string())
        );
    }
    ...
    fn test_legacy_tool_value_markdown_deserialization() {
        ...
        assert_eq!(
            actual.values[0],
            crate::domain::ToolValue::Text("# Heading - Some bold text".to_string())
        );
    }
    ...
    fn test_legacy_tool_value_file_diff_deserialization() {
        ...
        assert_eq!(
            actual.values[0],
            crate::domain::ToolValue::Text("[File diff: /src/main.rs]".to_string())
        );
    }
}
