...
use crate::app::{EnvironmentInfra, FileInfoInfra, FileReaderInfra, Walker, WalkerInfra};
...
impl<I: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra> ForgeSkillRepository<I> {
    ...
    fn render_skill(&self, skill: Skill, env: &crate::domain::Environment) -> Skill {
...
mod tests {
    ...
    fn fixture_skill_repo() -> (ForgeSkillRepository<ForgeInfra>, std::path::PathBuf) {
        let skill_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/repo/fixtures/skills_with_resources");
    ...
    async fn test_extract_skill_with_valid_metadata() {
        ...
        let content =
            fixture!("/src/repo/fixtures/skills/with_name_and_description.md").await;
    ...
    async fn test_extract_skill_with_incomplete_metadata() {
        // Fixture
        let content = fixture!("/src/repo/fixtures/skills/with_name_only.md").await;
