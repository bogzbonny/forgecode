...
use crate::app::{AgentRepository, DirectoryReaderInfra, EnvironmentInfra, FileInfoInfra};
use crate::config::ForgeConfig;
use crate::domain::{ModelId, ProviderId, Template, ToolName};
    ...
    AgentRepository for ForgeAgentRepository<F>
{
    async fn get_agents(&self) -> anyhow::Result<Vec<crate::domain::Agent>> {
        ...
        let session = config
            .session
            .clone()
            .ok_or(crate::domain::Error::NoDefaultSession)?;
    ...
    async fn get_agent_infos(&self) -> anyhow::Result<Vec<crate::domain::AgentInfo>> {
        ...
        Ok(agent_defs
            .into_iter()
            .filter(|def| filter_agent(def, &config))
            .map(|def| crate::domain::AgentInfo {
                id: def.id,
                title: def.title,
...
fn filter_agent(def: &AgentDefinition, config: &ForgeConfig) -> bool {
    if def.id.as_str() == crate::domain::AgentId::SAGE.as_str() && !config.research_subagent {
        return false;
    }
    true
}
...
mod tests {
    ...
    async fn test_parse_basic_agent() {
        let content = fixture!("/src/repo/fixtures/agents/basic.md").await;
    ...
    async fn test_parse_advanced_agent() {
        let content = fixture!("/src/repo/fixtures/agents/advanced.md").await;
