...
use chrono::Utc;
use crate::app::GrpcInfra;
use crate::domain::{
    ApiKey, FileUploadInfo, Node, UserId, WorkspaceAuth, WorkspaceId, WorkspaceIndexRepository,
    WorkspaceInfo,
};

use super::proto_generated::forge_service_client::ForgeServiceClient;
use super::proto_generated::{self, *};
...
impl<I: GrpcInfra> WorkspaceIndexRepository for ForgeContextEngineRepository<I> {
    ...
    async fn search(
        &self,
        search_query: &crate::domain::CodeSearchQuery<'_>,
        auth_token: &crate::domain::ApiKey,
    ) -> Result<Vec<Node>> {
            ...
            .filter_map(|query_item| {
                ...
                let code_node = match node_data.kind? {
                    node_data::Kind::FileChunk(chunk) => {
                        crate::domain::NodeData::FileChunk(crate::domain::FileChunk {
                    ...
                    node_data::Kind::File(file) => {
                        crate::domain::NodeData::File(crate::domain::FileNode {
                            file_path: file.path,
                    ...
                    node_data::Kind::FileRef(file_ref) => {
                        crate::domain::NodeData::FileRef(crate::domain::FileRef {
                            file_path: file_ref.path,
                            file_hash: file_ref.file_hash,
                        })
                    }
                    node_data::Kind::Note(note) => {
                        crate::domain::NodeData::Note(crate::domain::Note { content: note.content })
                    }
                    node_data::Kind::Task(task) => {
                        crate::domain::NodeData::Task(crate::domain::Task { task: task.task })
                    }
                };
    ...
    async fn list_workspace_files(
        ...
        auth_token: &crate::domain::ApiKey,
    ) -> Result<Vec<crate::domain::FileHash>> {
