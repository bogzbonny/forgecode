...
pub use crate::infra::CacacheStorage;
use reqwest::Response;
use reqwest::header::HeaderMap;
use reqwest_eventsource::EventSource;
use url::Url;

use super::agent::ForgeAgentRepository;
use super::context_engine::ForgeContextEngineRepository;
use crate::repo::conversation::ConversationRepositoryImpl;
use super::database::{DatabasePool, PoolConfig};
use super::fs_snap::ForgeFileSnapshotService;
use super::fuzzy_search::ForgeFuzzySearchRepository;
use crate::repo::provider::{ForgeChatRepository, ForgeProviderRepository};
use super::skill::ForgeSkillRepository;
use super::validation::ForgeValidationRepository;
...
impl<
    F: EnvironmentInfra<Config = crate::config::ForgeConfig>
        + FileReaderInfra
        + FileWriterInfra
        + GrpcInfra
        + HttpInfra,
...
> ProviderRepository for ForgeRepo<F>
{
    ...
    async fn get_provider(&self, id: ProviderId) -> anyhow::Result<crate::domain::ProviderTemplate> {
...
#[async_trait::async_trait]
impl<F: EnvironmentInfra<Config = crate::config::ForgeConfig> + Send + Sync> EnvironmentInfra
    for ForgeRepo<F>
{
    type Config = crate::config::ForgeConfig;

    fn get_environment(&self) -> Environment {
        self.infra.get_environment()
    }

    fn get_config(&self) -> anyhow::Result<crate::config::ForgeConfig> {
        self.infra.get_config()
    }

    fn update_environment(
        &self,
        ops: Vec<crate::domain::ConfigOperation>,
...
{
    async fn get_agents(&self) -> anyhow::Result<Vec<crate::domain::Agent>> {
        self.agent_repository.get_agents().await
    }

    async fn get_agent_infos(&self) -> anyhow::Result<Vec<crate::domain::AgentInfo>> {
...
impl<F: StrategyFactory> StrategyFactory for ForgeRepo<F> {
    ...
    fn create_auth_strategy(
        ...
        auth_method: crate::domain::AuthMethod,
        required_params: Vec<crate::domain::URLParamSpec>,
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> crate::domain::WorkspaceIndexRepository for ForgeRepo<F> {
    async fn authenticate(&self) -> anyhow::Result<crate::domain::WorkspaceAuth> {
    ...
    async fn create_workspace(
        ...
        auth_token: &crate::domain::ApiKey,
    ) -> anyhow::Result<crate::domain::WorkspaceId> {
        self.codebase_repo
    ...
    async fn upload_files(
        &self,
        upload: &crate::domain::FileUpload,
        auth_token: &crate::domain::ApiKey,
    ) -> anyhow::Result<crate::domain::FileUploadInfo> {
        self.codebase_repo.upload_files(upload, auth_token).await
    }

    async fn search(
        &self,
        query: &crate::domain::CodeSearchQuery<'_>,
        auth_token: &crate::domain::ApiKey,
    ) -> anyhow::Result<Vec<crate::domain::Node>> {
    ...
    async fn list_workspaces(
        &self,
        auth_token: &crate::domain::ApiKey,
    ) -> anyhow::Result<Vec<crate::domain::WorkspaceInfo>> {
    ...
    async fn get_workspace(
        ...
        auth_token: &crate::domain::ApiKey,
    ) -> anyhow::Result<Option<crate::domain::WorkspaceInfo>> {
        self.codebase_repo
    ...
    async fn list_workspace_files(
        &self,
        workspace: &crate::domain::WorkspaceFiles,
        auth_token: &crate::domain::ApiKey,
    ) -> anyhow::Result<Vec<crate::domain::FileHash>> {
        self.codebase_repo
    ...
    async fn delete_files(
        &self,
        deletion: &crate::domain::FileDeletion,
        auth_token: &crate::domain::ApiKey,
    ) -> anyhow::Result<()> {
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> crate::domain::ValidationRepository for ForgeRepo<F> {
    async fn validate_file(
        &self,
        path: impl AsRef<std::path::Path> + Send,
        content: &str,
    ) -> anyhow::Result<Vec<crate::domain::SyntaxError>> {
...
impl<F: crate::domain::ConsoleWriter> crate::domain::ConsoleWriter for ForgeRepo<F> {
