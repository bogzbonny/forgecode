...
use crate::config::{ForgeConfig, TlsBackend, TlsVersion};
...
impl<F: crate::app::FileWriterInfra + 'static> ForgeHttpInfra<F> {
    /// Creates a new [`ForgeHttpInfra`] from a resolved [`ForgeConfig`].
    pub fn new(config: ForgeConfig, file_writer: Arc<F>) -> Self {
        let http = config.http.unwrap_or(crate::config::HttpConfig {
...
impl<F: crate::app::FileWriterInfra + 'static> HttpInfra for ForgeHttpInfra<F> {
