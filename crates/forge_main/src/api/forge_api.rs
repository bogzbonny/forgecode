...
use crate::domain::{Agent, ConsoleWriter, *};
...
impl<A, F> ForgeAPI<A, F> {
    ...
    where
        A: Services + EnvironmentInfra<Config = crate::config::ForgeConfig>,
...
impl ForgeAPI<ForgeServices<ForgeRepo<ForgeInfra>>, ForgeRepo<ForgeInfra>> {
    ...
    pub async fn get_skills_internal(&self) -> Result<Vec<Skill>> {
        use crate::domain::SkillRepository;
...
> API for ForgeAPI<A, F>
{
    ...
    async fn update_config(&self, ops: Vec<crate::domain::ConfigOperation>) -> anyhow::Result<()> {
        ...
        let needs_agent_reload = ops
            .iter()
            .any(|op| matches!(op, crate::domain::ConfigOperation::SetSessionConfig(_)));
    ...
    async fn user_usage(&self) -> Result<Option<UserUsage>> {
        ...
        if let Some(api_key) = provider
            ...
            .and_then(|c| match &c.auth_details {
                crate::domain::AuthDetails::ApiKey(key) => Some(key.as_str()),
                _ => None,
            })
        {
    ...
    async fn generate_command(&self, prompt: UserPrompt) -> Result<String> {
        use crate::app::CommandGenerator;
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> Result<MpscStream<Result<crate::domain::SyncProgress>>> {
    ...
    ) -> Result<Vec<crate::domain::Node>> {
        self.services.query_workspace(path, params).await
    }

    async fn list_workspaces(&self) -> Result<Vec<crate::domain::WorkspaceInfo>> {
    ...
    async fn get_workspace_info(
        &self,
        path: PathBuf,
    ) -> Result<Option<crate::domain::WorkspaceInfo>> {
        self.services.get_workspace_info(path).await
    }

    async fn delete_workspaces(&self, workspace_ids: Vec<crate::domain::WorkspaceId>) -> Result<()> {
        self.services.delete_workspaces(&workspace_ids).await
    }

    async fn get_workspace_status(&self, path: PathBuf) -> Result<Vec<crate::domain::FileStatus>> {
    ...
    async fn create_auth_credentials(&self) -> Result<crate::domain::WorkspaceAuth> {
        self.services.init_auth_credentials().await
    }

    async fn init_workspace(&self, path: PathBuf) -> Result<crate::domain::WorkspaceId> {
        self.services.init_workspace(path).await
    }

    async fn migrate_env_credentials(&self) -> Result<Option<crate::domain::MigrationResult>> {
    ...
    async fn get_session_config(&self) -> Option<crate::domain::ModelConfig> {
    ...
    async fn get_default_provider(&self) -> Result<Provider<Url>> {
        let model_config = self
            ...
            .get_session_config()
            .await
            .ok_or_else(|| crate::domain::Error::NoDefaultSession)?;
    ...
    async fn mcp_auth(&self, server_url: &str) -> Result<()> {
        ...
        crate::infra::mcp_auth(server_url, &env).await
    }

    async fn mcp_logout(&self, server_url: Option<&str>) -> Result<()> {
        let env = self.services.get_environment().clone();
        match server_url {
            Some(url) => crate::infra::mcp_logout(url, &env).await,
            None => crate::infra::mcp_logout_all(&env).await,
        }
    }

    async fn mcp_auth_status(&self, server_url: &str) -> Result<String> {
        let env = self.services.get_environment().clone();
        Ok(crate::infra::mcp_auth_status(server_url, &env).await)
    }
