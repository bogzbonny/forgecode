...
use crate::domain::{Context, Error, Metrics, Result, TokenCount};
...
impl Conversation {
    ...
    pub fn to_html(&self) -> String {
        ...
        crate::domain::conversation_html::render_conversation_html(self)
    }
    ...
    pub fn to_html_with_related(&self, related: &[Conversation]) -> String {
        crate::domain::conversation_html::render_conversation_html_with_related(self, related)
    }

    /// Returns a vector of user messages, selecting the first message from
    /// each consecutive sequence of user messages.
    pub fn first_user_messages(&self) -> Vec<&crate::domain::ContextMessage> {
        self.context
            .as_ref()
    ...
    /// if available.
    pub fn accumulated_usage(&self) -> Option<crate::domain::Usage> {
        self.context.as_ref().and_then(|ctx| ctx.accumulate_usage())
    }

    pub fn usage(&self) -> Option<crate::domain::Usage> {
        self.context
            .iter()
        ...
        self.context
            ...
            .map(|ctx| {
                ctx.messages
                    ...
                    .filter_map(|value| {
                        if let crate::domain::ToolValue::AI { conversation_id, .. } = value {
...
mod tests {
    ...
    use super::*;
    use crate::domain::{Context, ContextMessage, ToolOutput, ToolResult, ToolValue};
    ...
    #[test]
    fn test_total_cost() {
        use crate::domain::{MessageEntry, Usage};
