...
use crate::domain::reasoning::{Reasoning, ReasoningFull};
use crate::domain::{
...
#[async_trait::async_trait]
impl ResultStreamExt<anyhow::Error> for crate::domain::BoxStream<ChatCompletionMessage, anyhow::Error> {
    async fn into_full(
        self,
        should_interrupt_for_xml: bool,
    ...
    async fn into_full_streaming(
        ...
        let partial_tool_calls = ToolCallFull::try_from_parts(&tool_call_parts)
            ...
            .map_err(crate::domain::Error::Retryable)?;
        ...
        {
            return Err(crate::domain::Error::EmptyCompletion.into_retryable().into());
        }
...
mod tests {
    ...
    async fn test_into_full_with_tool_call_parse_failure_creates_retryable_error() {
        use crate::domain::{ToolCallId, ToolCallPart, ToolName};
    ...
    async fn test_into_full_with_reasoning_details() {
        ...
        let reasoning_part = crate::domain::reasoning::ReasoningPart {
