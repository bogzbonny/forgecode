...
impl Agent {
    ...
    pub fn tool_order(&self) -> crate::domain::ToolOrder {
        self.tools
            .as_ref()
            .map(|tools| crate::domain::ToolOrder::from_tool_list(tools))
            .unwrap_or_default()
    }
}
...
impl From<Agent> for ToolDefinition {
    fn from(value: Agent) -> Self {
        ...
        ToolDefinition {
            name,
            description,
            input_schema: schemars::schema_for!(crate::domain::AgentInput),
        }
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::domain::{InputModality, Model};
