...
fn create_message_usage_section(usage: &crate::domain::message::Usage) -> Element {
    let cache_percentage = if *usage.prompt_tokens > 0 {
        (*usage.cached_tokens as f64 / *usage.prompt_tokens as f64 * 100.0) as usize
    } else {
        0
                        ...
                        Element::new(message_class)
                            ...
                            .append(Element::new("div.main-content").append(
                                tool_result.output.values.iter().filter_map(|value| {
                                    match value {
                                        crate::domain::ToolValue::Text(text) => Some(
                                            Element::new("div")
                                                .append(Element::new("pre").text(text)),
                                        ),
                                        ...
                                        crate::domain::ToolValue::Empty => None,
                                        crate::domain::ToolValue::AI { value, conversation_id } => {
...
mod tests {
    ...
    async fn test_render_conversation_html_snapshot() {
        ...
        let conversation: Conversation = json_fixture!("/src/app/fixtures/conversation.json").await;
