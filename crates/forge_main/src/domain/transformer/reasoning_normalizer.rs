use crate::domain::{Context, ModelId, Transformer};
...
impl Transformer for ReasoningNormalizer {
    ...
    fn transform(&mut self, mut context: Self::Value) -> Self::Value {
        ...
        let cutoff = context
            ...
            .rev()
            .find_map(|(idx, msg)| {
                if msg.has_role(crate::domain::Role::Assistant)
                    && let crate::domain::ContextMessage::Text(text) = &**msg
        ...
        for (idx, message) in context.messages.iter_mut().enumerate() {
            ...
            if message.has_role(crate::domain::Role::Assistant)
                && let crate::domain::ContextMessage::Text(text_msg) = &mut **message
            {
...
mod tests {
    ...
    use super::*;
    use crate::domain::{ContextMessage, ReasoningConfig, ReasoningFull, Role, TextMessage};
    ...
    fn test_strips_all_reasoning_when_model_changed() {
        ...
        for message in &actual.messages {
            if message.has_role(Role::Assistant)
                && let crate::domain::ContextMessage::Text(text) = &**message
            {
                assert_eq!(
    ...
    fn test_a_to_b_stay_on_b_strips_a_keeps_b() {
        ...
        if let crate::domain::ContextMessage::Text(a1) = &**msgs[1] {
            assert_eq!(
        ...
        if let crate::domain::ContextMessage::Text(a2) = &**msgs[3] {
            assert_eq!(
    ...
    fn test_alternating_a_b_a_stay_a_strips_ab_keeps_last_a() {
        ...
        if let crate::domain::ContextMessage::Text(b2) = &**msgs[3] {
            assert_eq!(
        ...
        if let crate::domain::ContextMessage::Text(a3) = &**msgs[5] {
            assert_eq!(
    ...
    fn test_mixed_sequence_preserves_only_same_model_tail() {
        ...
        for tail_msg in &assistant_msgs[assistant_msgs.len() - 3..] {
            if let crate::domain::ContextMessage::Text(t) = &***tail_msg {
                assert_eq!(
                    t.reasoning_details,
                    Some(reasoning_details()),
        ...
        for pre_msg in &assistant_msgs[..assistant_msgs.len() - 3] {
            if let crate::domain::ContextMessage::Text(t) = &***pre_msg {
                assert_eq!(
