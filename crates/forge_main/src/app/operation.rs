...
use crate::app::utils::{compute_hash, format_display_path};
use crate::app::{
...
pub enum ToolOperation {
    ...
    Skill {
        output: crate::domain::Skill,
    },
    TodoWrite {
        before: Vec<crate::domain::Todo>,
        after: Vec<crate::domain::Todo>,
    },
    TodoRead {
        output: Vec<crate::domain::Todo>,
    },
}
...
fn create_validation_warning(path: &str, errors: &[crate::domain::SyntaxError]) -> Element {
    Element::new("warning")
...
impl ToolOperation {
    /// Converts this tool operation into a [`crate::domain::ToolOutput`].
    ///
    /// # Arguments
        ...
        match self {
            ToolOperation::FsRead { input, output } => {
                ...
                if let Some(image) = output.content.as_image() {
                    ...
                    *metrics = metrics.clone().insert(
                    ...
                    return crate::domain::ToolOutput::image(image.clone());
                }
                ...
                *metrics = metrics.clone().insert(
                ...
                crate::domain::ToolOutput::text(elm)
            }
            ...
            ToolOperation::FsRemove { input, output } => {
                ...
                *metrics = metrics.clone().insert(
                ...
                crate::domain::ToolOutput::text(elem)
            }
            ...
            ToolOperation::CodebaseSearch { output } => {
                ...
                if output.queries.is_empty() || total_results == 0 {
                    ...
                    for query_result in &output.queries {
                        ...
                        for data in &query_result.results {
                            if let crate::domain::NodeData::FileChunk(file_chunk) = &data.node {
                ...
                crate::domain::ToolOutput::text(root)
            }
            ...
            ToolOperation::Shell { output } => {
                ...
                crate::domain::ToolOutput::text(parent_elem)
            }
            ...
            ToolOperation::TodoWrite { before, after } => {
                ...
                let before_map: std::collections::HashMap<&str, &crate::domain::Todo> =
...
mod tests {
    ...
    use crate::domain::{FSRead, FileInfo, ToolValue};

    use super::*;
    use crate::app::{Content, Match, MatchResult};
    ...
    fn to_value(output: crate::domain::ToolOutput) -> String {
    ...
    fn test_syntax_errors(errors: Vec<(u32, u32, &str)>) -> Vec<crate::domain::SyntaxError> {
        use crate::domain::SyntaxError;
    ...
    mod sem_search_helpers {
        use fake::{Fake, Faker};
        use crate::domain::{CodebaseQueryResult, CodebaseSearchResults, FileChunk, Node, NodeData};
    ...
    fn test_shell_output_no_truncation() {
        let fixture = ToolOperation::Shell {
            output: ShellOutput {
                output: crate::domain::CommandOutput {
    ...
    fn test_fs_remove_success() {
        let fixture = ToolOperation::FsRemove {
            input: crate::domain::FSRemove { path: "/home/user/file_to_delete.txt".to_string() },
    ...
    fn test_fs_search_files_with_matches_mode() {
        let fixture = ToolOperation::FsSearch {
            input: crate::domain::FSSearch {
                ...
                output_mode: Some(crate::domain::OutputMode::FilesWithMatches),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_search_count_mode() {
        let fixture = ToolOperation::FsSearch {
            input: crate::domain::FSSearch {
                path: Some("/home/user/project".to_string()),
                pattern: "test".to_string(),
                output_mode: Some(crate::domain::OutputMode::Count),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_undo_no_changes() {
        let fixture = ToolOperation::FsUndo {
            input: crate::domain::FSUndo { path: "/home/user/unchanged_file.txt".to_string() },
    ...
    fn test_fs_undo_file_created() {
        let fixture = ToolOperation::FsUndo {
            input: crate::domain::FSUndo { path: "/home/user/new_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: None,
    ...
    fn test_fs_undo_file_removed() {
        let fixture = ToolOperation::FsUndo {
            input: crate::domain::FSUndo { path: "/home/user/deleted_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: Some(
    ...
    fn test_fs_undo_file_restored() {
        let fixture = ToolOperation::FsUndo {
            input: crate::domain::FSUndo { path: "/home/user/restored_file.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_fs_undo_success() {
        let fixture = ToolOperation::FsUndo {
            input: crate::domain::FSUndo { path: "/home/user/test.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_fs_read_image_with_vision_model() {
        ...
        match &actual.values[0] {
            crate::domain::ToolValue::Image(_) => (), // Expected
