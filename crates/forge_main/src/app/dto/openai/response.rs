...
use crate::app::dto::openai::ReasoningDetail;
use crate::app::dto::openai::error::{Error, ErrorCode, ErrorResponse};
...
impl From<ReasoningDetail> for crate::domain::ReasoningDetail {
    fn from(detail: ReasoningDetail) -> Self {
        crate::domain::ReasoningDetail {
            text: detail.text,
...
impl GitHubCopilotReasoning {
    fn into_reasoning_detail(self) -> crate::domain::ReasoningDetail {
        crate::domain::ReasoningDetail {
            text: self.text,
            data: self.data,
            type_of: self.r#type,
                    ...
                    let mut response = match choice {
                        ...
                        Choice::NonStreaming { message, finish_reason, .. } => {
                            ...
                            if let Some(reasoning_details) = &message.reasoning_details {
                                let converted_details: Vec<crate::domain::ReasoningFull> =
                                    reasoning_details
                                        .clone()
                                        .into_iter()
                                        .map(crate::domain::ReasoningFull::from)
                                        .collect();

                                resp = resp.add_reasoning_detail(crate::domain::Reasoning::Full(
                                    converted_details,
                                ));
                                ...
                                for tool_call in tool_calls {
                                    ...
                                    resp = resp.add_tool_call(ToolCallFull {
                                        ...
                                        name: tool_call
                                            ...
                                            .name
                                            .clone()
                                            .ok_or(crate::domain::Error::ToolCallMissingName)?,
                        ...
                        Choice::Streaming { delta, finish_reason, .. } => {
                            ...
                            if let Some(reasoning_details) = &delta.reasoning_details {
                                let converted_details: Vec<crate::domain::ReasoningPart> =
                                    reasoning_details
                                        .clone()
                                        .into_iter()
                                        .map(crate::domain::ReasoningPart::from)
                                        .collect();
                                resp = resp.add_reasoning_detail(crate::domain::Reasoning::Part(
                                    converted_details,
                                ));
                            } else if let Some(details) = convert_github_copilot_reasoning(
...
mod tests {
    use anyhow::Context;
    use crate::domain::ChatCompletionMessage;
    use crate::fixture;
    ...
    async fn load_fixture(filename: &str) -> serde_json::Value {
        let fixture_path = format!("src/app/dto/openai/fixtures/{}", filename);
    ...
    #[test]
    fn test_generic_provider_response_event_missing_arguments() {
        let event = "{\"id\":\"gen-1749331907-SttL6PXleVHnrdLMABfU\",\"provider\":\"generic\",\"model\":\"qwen/qwen3-235b-a22b\",\"object\":\"chat.completion.chunk\",\"created\":1749331907,\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\",\"content\":null,\"tool_calls\":[{\"index\":0,\"id\":\"call_Wl2L8rrzHwrXSeiciIvU65IS\",\"type\":\"function\",\"function\":{\"name\":\"attempt_completion\"}}]},\"finish_reason\":null,\"native_finish_reason\":null,\"logprobs\":null}]}";
        assert!(Fixture::test_response_compatibility(event));
    }

    #[tokio::test]
    async fn test_responses() -> anyhow::Result<()> {
        let content = fixture!("/src/app/dto/openai/responses.jsonl").await;
