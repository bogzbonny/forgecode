...
impl From<Model> for crate::domain::Model {
    fn from(value: Model) -> Self {
        ...
        let input_modalities = value
            ...
            .map(|modalities| {
                modalities
                    .iter()
                    .filter_map(|s| s.parse::<crate::domain::InputModality>().ok())
                    .collect::<Vec<_>>()
            })
            .unwrap_or_else(|| vec![crate::domain::InputModality::Text]);
...
mod tests {
    ...
    async fn test_model_conversion_without_supported_parameters() {
        ...
        let domain_model: crate::domain::Model = model.into();
