...
mod tests {
    ...
    use crate::domain::{FileOperation, Metrics, ToolKind};
    ...
    impl FsReadService for MockFsReadService {
        async fn read(
            ...
            _: Option<u64>,
            _: Option<u64>,
        ) -> anyhow::Result<crate::app::ReadOutput> {
            ...
            if let Some(file) = self.files.get(&path) {
                Ok(crate::app::ReadOutput {
                    content: Content::File(file.displayed_content.clone()),
                    info: crate::domain::FileInfo::new(1, 1, 1, compute_hash(&file.raw_content)),
                })
            } else {
