...
use crate::app::services::{AgentRegistry, CustomInstructionsService, ProviderAuthService};
use crate::app::set_conversation_id::SetConversationId;
use crate::app::system_prompt::SystemPrompt;
use crate::app::tool_registry::ToolRegistry;
use crate::app::tool_resolver::ToolResolver;
...
pub(crate) fn build_template_config(config: &ForgeConfig) -> crate::domain::TemplateConfig {
    crate::domain::TemplateConfig {
...
impl<S: Services + EnvironmentInfra<Config = crate::config::ForgeConfig>> ForgeApp<S> {
    ...
    pub async fn chat(
        ...
        let conversation = services
            ...
            .await?
            .ok_or_else(|| crate::domain::Error::ConversationNotFound(chat.conversation_id))?;
        ...
        let agent = self
            ...
            .get_agent(&agent_id)
            .await?
            .ok_or(crate::app::Error::AgentNotFound(agent_id.clone()))?
    ...
    pub async fn compact_conversation(
        ...
        use crate::app::compact::Compactor;
        ...
        let mut conversation = self
            ...
            .await?
            .ok_or_else(|| crate::domain::Error::ConversationNotFound(*conversation_id))?;
