...
use crate::app::agent_executor::AgentExecutor;
use crate::app::dto::ToolsOverview;
use crate::app::error::Error;
use crate::app::fmt::content::FormatContent;
use crate::app::mcp_executor::McpExecutor;
use crate::app::tool_executor::ToolExecutor;
use crate::app::{
...
impl<S: Services + EnvironmentInfra<Config = crate::config::ForgeConfig>> ToolRegistry<S> {
        ...
        if ToolCatalog::contains(&input.name) {
            ...
            if let ToolCatalog::Task(task_input) = tool_input {
                ...
                let conversation_id = session_id
                    .map(|id| crate::domain::ConversationId::parse(&id))
                    .transpose()
                    .ok()
                    .flatten();
            ...
            if is_restricted && self.check_tool_permission(&tool_input, context).await? {
                ...
                context
                    .send(crate::domain::TitleFormat::error("Permission Denied"))
                    .await?;
...
mod tests {
    ...
    use crate::app::error::Error;
    use crate::app::tool_registry::{ToolRegistry, create_test_agents};
...
#[cfg(test)]
fn create_test_agents() -> Vec<crate::domain::Agent> {
    use crate::domain::{Agent, AgentId, ModelId, ProviderId, ToolName};
...
fn create_test_model(
    id: &str,
    modalities: Vec<crate::domain::InputModality>,
) -> crate::domain::Model {
...
fn test_dynamic_tool_description_with_vision_model() {
    use fake::{Fake, Faker};
    use crate::domain::InputModality;
...
fn test_validate_tool_modality_with_image_file_and_vision_model() {
    use crate::domain::{InputModality, ToolCatalog};

    let vision_model = create_test_model("gpt-4o", vec![InputModality::Text, InputModality::Image]);
    let tool_input = ToolCatalog::Read(crate::domain::FSRead {
...
fn test_validate_tool_modality_with_non_read_tool() {
    ...
    let text_only_model = create_test_model("gpt-3.5-turbo", vec![InputModality::Text]);
    let tool_input = ToolCatalog::Write(crate::domain::FSWrite {
        file_path: "/home/user/test.png".to_string(),
