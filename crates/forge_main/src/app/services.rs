...
use crate::app::user::{User, UserUsage};
use crate::app::{EnvironmentInfra, Walker};
...
pub struct TodoWriteOutput {
    ...
    pub todos: Vec<crate::domain::Todo>,
}
...
pub trait ProviderService: Send + Sync {
    ...
    async fn get_provider(&self, id: crate::domain::ProviderId) -> anyhow::Result<Provider<Url>>;
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>>;
    async fn upsert_credential(
        &self,
        credential: crate::domain::AuthCredential,
    ) -> anyhow::Result<()>;
    async fn remove_credential(&self, id: &crate::domain::ProviderId) -> anyhow::Result<()>;
...
pub trait AppConfigService: Send + Sync {
    ...
    async fn get_reasoning_effort(&self) -> anyhow::Result<Option<crate::domain::Effort>>;
    ...
    /// all configuration changes; use [`crate::domain::ConfigOperation`]
...
pub trait FsPatchService: Send + Sync {
    ...
    async fn multi_patch(
        &self,
        path: String,
        edits: Vec<crate::domain::PatchEdit>,
    ) -> anyhow::Result<PatchOutput>;
}
...
pub trait ImageReadService: Send + Sync {
    ...
    async fn read_image(&self, path: String) -> anyhow::Result<crate::domain::Image>;
}
...
pub trait FsSearchService: Send + Sync {
    ...
    /// * `Err(_)` - Search error
    async fn search(&self, params: crate::domain::FSSearch) -> anyhow::Result<Option<SearchResult>>;
}
...
pub trait AgentRegistry: Send + Sync {
    ...
    async fn get_agent(&self, agent_id: &AgentId) -> anyhow::Result<Option<crate::domain::Agent>>;
...
pub trait CommandLoaderService: Send + Sync {
    ...
    async fn get_commands(&self) -> anyhow::Result<Vec<crate::domain::Command>>;
}
...
pub trait PolicyService: Send + Sync {
    ...
    async fn check_operation_permission(
        &self,
        operation: &crate::domain::PermissionOperation,
...
pub trait SkillFetchService: Send + Sync {
    ...
    async fn fetch_skill(&self, skill_name: String) -> anyhow::Result<crate::domain::Skill>;
    ...
    async fn list_skills(&self) -> anyhow::Result<Vec<crate::domain::Skill>>;
}
