...
impl<S: FsReadService + EnvironmentInfra<Config = crate::config::ForgeConfig>> ChangedFiles<S> {
    ...
    pub async fn update_file_stats(&self, mut conversation: Conversation) -> Conversation {
        use crate::app::file_tracking::FileChangeDetector;
...
mod tests {
    ...
    use super::*;
    use crate::app::services::Content;
    use crate::app::{FsReadService, ReadOutput, compute_hash};
            ...
            self.files
                ...
                .map(|content| {
                    ...
                    ReadOutput {
                        ...
                        info: crate::domain::FileInfo::new(1, 1, 1, hash),
                    }
                })
    ...
    impl EnvironmentInfra for TestServices {
        ...
        fn get_config(&self) -> anyhow::Result<crate::config::ForgeConfig> {
            Ok(crate::config::ForgeConfig { max_parallel_file_reads: 4, ..Default::default() })
        }
