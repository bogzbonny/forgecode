...
use crate::app::{AgentRegistry, ConversationService, EnvironmentInfra, Services};
#[derive(Clone)]
...
impl<S: Services + EnvironmentInfra<Config = crate::config::ForgeConfig>> AgentExecutor<S> {
    ...
    pub async fn execute(
        ...
        let conversation = if let Some(conversation_id) = conversation_id {
            ...
            let context = crate::domain::Context::default().initiator("agent".to_string());
        ...
        let app = crate::app::ForgeApp::new(self.services.clone());
