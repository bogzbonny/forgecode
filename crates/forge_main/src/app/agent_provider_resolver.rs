...
use anyhow::Result;
use crate::domain::{AgentId, ModelId, Provider};

use crate::app::{AgentRegistry, AppConfigService, ProviderAuthService, ProviderService};
    ...
    pub async fn get_provider(&self, agent_id: Option<AgentId>) -> Result<Provider<url::Url>> {
        let provider_id = if let Some(agent_id) = agent_id {
            ...
            if let Some(agent) = self.0.get_agent(&agent_id).await? {
                ...
                // we can throw crate::app::Error::AgentNotFound
                self.0
                    .get_session_config()
                    .await
                    .map(|c| c.provider)
        ...
        if let Some(agent_id) = agent_id {
            if let Some(agent) = self.0.get_agent(&agent_id).await? {
                ...
                self.0
                    ...
                    .await
                    .map(|c| c.model)
                    .ok_or_else(|| crate::domain::Error::NoDefaultSession.into())
            }
        } else {
            self.0
                .get_session_config()
