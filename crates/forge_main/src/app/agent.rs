...
use crate::app::services::AppConfigService;
use crate::app::tool_registry::ToolRegistry;
use crate::app::{ConversationService, EnvironmentInfra, ProviderService, Services};
...
#[async_trait::async_trait]
impl<T: Services + EnvironmentInfra<Config = crate::config::ForgeConfig>> AgentService for T {
    async fn chat_agent(
        &self,
        id: &ModelId,
        context: Context,
...
impl AgentExt for Agent {
    fn apply_config(self, config: &ForgeConfig) -> Agent {
        ...
        if let Some(ref workflow_compact) = config.compact {
            // Convert crate::config::Compact to crate::domain::Compact, then merge.
        ...
        if let Some(ref config_reasoning) = config.reasoning {
            use crate::config::Effort as ConfigEffort;
...
#[cfg(test)]
mod tests {
    use crate::config::{Effort as ConfigEffort, ReasoningConfig as ConfigReasoningConfig};
    use crate::domain::{AgentId, Effort, ModelId, ProviderId, ReasoningConfig};
    ...
    fn test_compact_agent_settings_take_priority_over_workflow_config() {
        ...
        let workflow_compact = crate::config::Compact::default()
    ...
    fn test_compact_partial_agent_settings_override_workflow_values() {
        use crate::config::Percentage;
        use crate::domain::Compact as DomainCompact;
