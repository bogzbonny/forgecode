...
where
    S: EnvironmentInfra<Config = crate::config::ForgeConfig>
...
mod tests {
    ...
    impl ProviderService for MockServices {
        ...
        async fn models(&self, _provider: Provider<Url>) -> Result<Vec<crate::domain::Model>> {
            Ok(vec![])
        }
        ...
        async fn get_all_providers(&self) -> Result<Vec<crate::domain::AnyProvider>> {
            Ok(vec![])
        }
    ...
    impl AppConfigService for MockServices {
        async fn get_session_config(&self) -> Option<crate::domain::ModelConfig> {
            Some(crate::domain::ModelConfig::new(
                ProviderId::OPENAI_COMPATIBLE,
                ModelId::new("test-model"),
            ))
        }

        async fn get_commit_config(&self) -> Result<Option<crate::domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_suggest_config(&self) -> Result<Option<crate::domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_reasoning_effort(&self) -> Result<Option<crate::domain::Effort>> {
            Ok(None)
        }

        async fn update_config(&self, _ops: Vec<crate::domain::ConfigOperation>) -> Result<()> {
            Ok(())
        }
    }
