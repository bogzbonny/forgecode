...
use crate::app::{CommandInfra, EnvironmentInfra, FileReaderInfra, WalkerInfra, WorkspaceService};
use crate::domain::{
...
use crate::services::fd::FileDiscovery;
use crate::services::sync::{WorkspaceSyncEngine, canonicalize_path};
...
> ForgeWorkspaceService<F, D>
{
    ...
    async fn get_workspace_credentials(&self) -> Result<(crate::domain::ApiKey, UserId)> {
        let credential = self
            .infra
    ...
    async fn find_workspace_by_path(
        ...
        let mut best_match: Option<(&crate::domain::WorkspaceInfo, usize)> = None;
    ...
    async fn get_workspace_by_path(
        ...
        path: PathBuf,
        token: &crate::domain::ApiKey,
    ) -> Result<crate::domain::WorkspaceInfo> {
...
> WorkspaceService for ForgeWorkspaceService<F, D>
{
    ...
    async fn query_workspace(
        ...
        let workspace = self
            ...
            .await?
            .ok_or(crate::domain::Error::WorkspaceNotFound)?;

        let search_query =
            crate::domain::CodeBase::new(user_id, workspace.workspace_id.clone(), params);
    ...
    async fn delete_workspace(&self, workspace_id: &crate::domain::WorkspaceId) -> Result<()> {
        let (token, _) = self.get_workspace_credentials().await?;
    ...
    /// database.
    async fn delete_workspaces(&self, workspace_ids: &[crate::domain::WorkspaceId]) -> Result<()> {
    ...
    async fn init_auth_credentials(&self) -> Result<crate::domain::WorkspaceAuth> {
    ...
    async fn init_workspace(&self, path: PathBuf) -> Result<WorkspaceId> {
        ...
        if is_new {
            ...
            Err(crate::domain::Error::WorkspaceAlreadyInitialized(workspace_id).into())
        }
    }
}
